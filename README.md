...
* Maintenance ("Filter") Script:
    > rt_tracker_filter.py
        * Prior to executing the maintenance script it is important to ensure the log that it reads from (log generated by rt_tracker_populate.py) contains 60 days worth of data -- that is, there must be at least N number of retrieval dates that are within the latest 60-day window from the time the the maintence is script is executed.   
